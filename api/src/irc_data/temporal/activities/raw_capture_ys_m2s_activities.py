"""Temporal activities for the DP-00-03 nightly raw-capture workflow.

Policy: v1.0 (DP-01-02; supersedes interim-v0 / DP-00-01)

Scope: Yacht Scoring + Manage2Sail raw archival capture (no parsing).

Activities
----------
  list_sources_activity      — return the DP-00-03 source slugs
...
bytes, conditional requests turn unchanged pages into HTTP 304 no-ops, and
``retrieval_events`` rows are append-only audit records.
"""
...
async def list_sources_activity() -> list[str]:
    """Return the source slugs this workflow is responsible for (DP-00-03)."""
    from irc_data.scrapers.raw_capture_ys_m2s import DP_00_03_SOURCES

    return list(DP_00_03_SOURCES)
...
async def capture_source_activity(
    ...
    from irc_data.scrapers.raw_capture_ys_m2s import run_nightly
    ...
    activity.logger.info(
        "capture_source_activity(dp-00-03): source=%s max_fetches=%d "
...
async def write_ledger_activity(summary: dict[str, Any]) -> str:
    """Persist the aggregated run ledger to ``ingestion_log``; return JSON.
    ...
    activity.logger.info(
        "Raw capture (DP-00-03) run complete: %s",
    ...
    try:
        ...
        with engine.begin() as conn:
            conn.execute(
                ...
                {
                    "source": summary.get("source_slug", "dp-00-03"),
                    ...
                    "metadata": _json.dumps(
                        {
                            ...
                            "canary": summary.get("canary", False),
