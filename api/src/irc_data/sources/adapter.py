...
# ---------------------------------------------------------------------------
# Checkpoint — backward-compatibility alias for AdapterCheckpointV1
# ---------------------------------------------------------------------------

# Legacy code (DP-01-01 adapters) used a simpler ``Checkpoint`` dataclass.
# We keep the name as an alias pointing at AdapterCheckpointV1 so existing
# imports don't break.  New code should use AdapterCheckpointV1 directly.
Checkpoint = AdapterCheckpointV1
...
class SourceAdapter(abc.ABC):
    ...
    * :meth:`parse_hint_for` — return the :class:`ParseHint` for a URL.
    ...
    # ------------------------------------------------------------------
    # Backward-compat: to_raw_artifact
    # ------------------------------------------------------------------
    ...
    def to_raw_artifact(
        ...
        """Convert a fetch result to a ``RawArtifactV1`` for storage.

        Backward-compatibility method retained for DP-01-01 adapters.
        New code should use :class:`RawCaptureRequestV1` directly.
        """
        try:
            from irc_data.sources.models import RawArtifactV1
            return RawArtifactV1.from_fetch_result(
                fetch_result, source_slug=self.source_slug, content_type=content_type
            )
        except (ImportError, AttributeError):
            # If models.py doesn't have RawArtifactV1, return the envelope as-is
            return fetch_result
